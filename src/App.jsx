...
function App() {
  ...
  return (
   <>
   <div className="app-container p-3 d-flex flex-column justify-content-center align-items-center">
    <h3>Todo List</h3>
    ...
    <div className="w-100 todos-container">
      <ul className='w-100'>
        {todos.map((todo)=>( <li className=' p-2 w-100 my-1 d-flex align-items-center justify-content-between'key={todo.id}>{todo.data}<span><button className='btn btn-success'  onClick={()=>{handleEdit(todo.id)}}>Edit</button><button className='btn btn-danger mx-1' onClick={()=>(handleDelete(todo.id))}>Delete</button></span></li>))}
      </ul>
    </div>
